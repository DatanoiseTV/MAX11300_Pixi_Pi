...
public:
  Pixi();
  ~Pixi();
  uint32_t ReadRegister(char address, bool debug);
  void WriteRegister(char address, uint32_t value);
  uint32_t config();
  uint32_t configChannel(int channel, int channel_mode, uint32_t dac_dat,
                         uint32_t range, char adc_ctl);
  //    void configTempChannel(int channel);
  //    void configInterrupt();
  //    void readInterrupt();
  uint32_t readRawTemperature(int temp_channel);
